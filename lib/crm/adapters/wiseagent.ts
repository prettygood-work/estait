...
export class WiseAgentAdapter implements CRMAdapter {
  ...
  private async makeRequest(
    ...
    throw (
      lastError ||
      new WiseAgentError('Request failed after retries', 'REQUEST_FAILED')
    );
  }
  ...
  async updateContact(
    ...
    if (data.address) {
      ...
      if (data.address.country !== undefined)
        params.Country = data.address.country;
    }
  ...
  async getNotes(userId: string, contactId: string): Promise<Note[]> {
    ...
    return response.map((note) => ({
      ...
      categories: note.Categories
        ? JSON.parse(note.Categories).map((cat: any) => cat.name)
        : [],
      createdAt: new Date(note.NoteDate),
      createdBy: note.insideteamid
        ? `Team Member ${note.insideteamid}`
        : undefined,
    }));
  }
  ...
  async generateSSOLink(userId: string, targetPage?: string): Promise<string> {
    ...
    return targetPage
      ? `${response}&page=${encodeURIComponent(targetPage)}`
      : response;
  }
  ...
  private mapWiseAgentContact(wiseAgentContact: any): Contact {
    ...
    return {
      ...
      createdAt: wiseAgentContact.DateAddedUTC
        ? new Date(wiseAgentContact.DateAddedUTC)
        : undefined,
      updatedAt: wiseAgentContact.DateUpdatedUTC
        ? new Date(wiseAgentContact.DateUpdatedUTC)
        : undefined,
      customFields: wiseAgentContact.CustomData
        ? JSON.parse(wiseAgentContact.CustomData).reduce(
            (acc: any, field: any) => {
