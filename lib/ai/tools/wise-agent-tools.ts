...
async function getWiseAgentAdapter(): Promise<CRMAdapter> {
  ...
  if (!isConnected) {
    throw new Error(
      'Wise Agent not connected. Please connect your account in settings.',
    );
  }
  return adapter;
}
...
export const createLead = tool({
  description:
    'Create a new lead/contact in Wise Agent CRM. Extract name, phone, email, and any other details from the user input.',
  inputSchema: z.object({
    ...
    address: z
      ...
      .optional()
      .describe('Address details if provided'),
    source: z.string().default('Estait Chat').describe('Lead source'),
    categories: z
      .array(z.string())
      .optional()
      .describe('Categories like Buyer, Seller, etc.'),
    status: z
      .string()
      .optional()
      .describe('Contact status like Hot Lead, Active, etc.'),
    notes: z
      .string()
      .optional()
      .describe('Any additional information about the lead'),
  }),
  execute: async (args) => {
...
export const addNote = tool({
  description:
    'Add a note to an existing contact in Wise Agent. Find the contact by name and add the note.',
  inputSchema: z.object({
    ...
    categories: z
      .array(z.string())
      .optional()
      .describe('Note categories like Buyer, Seller, Follow Up, etc.'),
  }),
...
export const searchContacts = tool({
  description:
    'Search for contacts in Wise Agent CRM by name, email, or other criteria.',
  inputSchema: z.object({
...
export const createTask = tool({
  description:
    'Create a task or reminder in Wise Agent, optionally linked to a contact.',
  inputSchema: z.object({
    ...
    dueDate: z
      ...
      .optional()
      .describe(
        'Due date in natural language (e.g., tomorrow, next week, 2024-12-25)',
      ),
    priority: z
...
export const linkPropertyToContact = tool({
  description:
    'Link a property from search results to a contact in Wise Agent by adding a detailed note.',
  inputSchema: z.object({
    ...
    clientInterest: z
      .string()
      .optional()
      .describe('Notes about client interest level'),
  }),
  execute: async ({
    contactName,
    propertyAddress,
...
export const getTeam = tool({
  description:
    'Get list of team members in Wise Agent for assignment purposes.',
  inputSchema: z.object({}),
  execute: async () => {
    const adapter = await getWiseAgentAdapter();
    const userId = await getCurrentUserId();
...
export const generateSSOLink = tool({
  description:
    'Generate a one-time login link to Wise Agent (valid for 60 seconds).',
  inputSchema: z.object({
    targetPage: z
      .string()
      .optional()
      .describe('Specific page to navigate to after login'),
  }),
